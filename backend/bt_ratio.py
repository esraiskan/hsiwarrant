...
def bt(d1, d15, allow_cum):
    ...
    for i in range(max(VOL_MA_PERIOD, RSI_LENGTH) + 1, len(d1)):
        ...
        if pos == "none":
            ...
            if not sig and allow_cum and abs(cum5) >= 40 and day_range >= 150:
                if cum5 < -40 and cs < 0 and day_trend < 0: sig = ("bear", "Cum")
                elif cum5 > 40 and cs > 0 and day_trend > 0: sig = ("bull", "Cum")
